...
public interface OutcomeRepository extends CrudRepository<Outcome, Long> {
    ...
    @Query("SELECT x.outcomeDate AS outcomeDate, SUM(x.amount) AS subTotal FROM Outcome x WHERE x.outcomeDate BETWEEN :fromDate AND :toDate GROUP BY x.outcomeDate ORDER BY x.outcomeDate ASC")
